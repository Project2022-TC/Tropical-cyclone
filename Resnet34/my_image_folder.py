...
def default_loader(path, size_m):
    image = Image.open(path)
    image_size = image.resize((size_m, size_m), Image.ANTIALIAS)
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root, size, transform=None, target_transform=default_transform,
    ...
    def __getitem__(self, index):
        ...
        img = self.loader(path, self.size)
